modulejs.define 'ViewPartialsQshop', [
	'Q'
	'jquery'
	'ViewPartialsFactory'
], (Q,$,ViewPartialsFactory) ->
	ViewPartialsQshop = ViewPartialsFactory.extend
		initialize: ->
			@.init(@.options.active)
		...
		renderDataTable: ->
			@.table = @.$el.find('#datatable-account-' + @.options.model.get('idutente') + '-qshop').dataTable
				aLengthMenu: [[20, 50, 100, -1], [20, 50, 100, "All"]]
				iDisplayLength: 20
				aaData: @.model.toJSON().qshop
				bDeferRender: true
				bRetrieve: true
				bIgnoreEmpty: false
				...
				aoColumns: do ->
					aNewData = [
						{'mData': (data, objs) -> '<a class="btn btn-mini btn-yellow openModal" data-toggle="modal" rel="' + data.idtransazione + '-' + data.idutente + '" href="#' + data.idtransazione + '"><i class="fontello-icon-accounts-qshop"></i>modifica</a>'}, 
